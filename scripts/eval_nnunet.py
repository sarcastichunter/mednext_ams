"""
eval_nnunet.py — Topološka evalvacija napovedi nnU-Net modela.

Skripta prebere nnU-Net napovedi (.nii.gz) in ground truth maske ter
izračuna Dice, HD95, Completeness, Correctness in Quality — enake metrike
kot run_test.py za MedNeXt. To omogoča neposredno primerjavo obeh modelov.

Uporaba:
    python3 eval_nnunet.py \
        --pred_path  predictions/nnunet \
        --label_path data/nnUNet_raw/Dataset001_ImageCAS/labelsTr \
        --output_path results/nnunet_metrics.json
"""
...
# -----------------------------------------------------------------------
# TOPOLOŠKE METRIKE (Heipke 1998) — enako kot v run_test.py
# -----------------------------------------------------------------------

def extract_centerline(mask_np: np.ndarray) -> np.ndarray:
    return skeletonize_3d(mask_np.astype(np.uint8))
...
def find_pairs(pred_path: str, label_path: str):
    """
    Poišče pare (napoved, ground truth) glede na skupno ime datoteke.
    nnU-Net napovedi so poimenovane enako kot labele: case_XXXX.nii.gz
    """
    pred_files  = {f for f in os.listdir(pred_path)  if f.endswith(".nii.gz")}
    label_files = {f for f in os.listdir(label_path) if f.endswith(".nii.gz")}

    common = sorted(pred_files & label_files)

    if not common:
        raise FileNotFoundError(
            f"Nobenih skupnih datotek med:\n  {pred_path}\n  {label_path}\n"
            "Preverite, da se imena ujemajo (npr. case_0001.nii.gz)."
        )

    return [
        (os.path.join(pred_path, f), os.path.join(label_path, f))
        for f in common
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Topološka + pikselska evalvacija nnU-Net napovedi."
    )
    parser.add_argument("--pred_path", type=str, required=True,
                        help="Mapa z nnU-Net napovedmi (case_XXXX.nii.gz)")
    parser.add_argument("--label_path", type=str, required=True,
                        help="Mapa z ground truth maskami (labelsTr)")
    parser.add_argument("--output_path", type=str, required=True,
                        help="Pot za shranjevanje metrik (JSON)")
    args = parser.parse_args()

    print("Iskanje parov napoved / ground truth...")
    pairs = find_pairs(args.pred_path, args.label_path)
    ...
    for i, (pred_file, label_file) in enumerate(pairs, 1):
        case_name = os.path.basename(pred_file)
        print(f"  [{i}/{len(pairs)}] {case_name}")
        ...
        # HD95 — samo če obe maski vsebujeta pozitivne voksle
